...
class WebsocketClient(object):
    ...
    def on_message(self, ws, message, *args):  # pylint: disable=unused-argument
        ...
        #######################################################
            ...
            self.api.subscribe_commission_changed_data[instrument_type][Active_name][
                self.api.timesync.server_timestamp] = int(commission)
        ...
        #**********************************************************
            ...
            self.handle_trade_result(message["msg"])
            ...
            for data in message["msg"]["quotes"]:
                ...
                for symble in data["symbols"]:
                    try:
                        ...
                        pass
            self.api.instrument_quites_generated_timestamp[Active_name][period] = message["msg"]["expiration"][
                "timestamp"]
    ...
    def handle_trade_result(self, msg):
        if 'close_profit' in msg:
            profit = msg['close_profit']
            if profit > 0:
                self.previous_trade_result = "win"
            else:
                self.previous_trade_result = "loss"
            #print(f"Trade précédent : {self.previous_trade_result}")
    ...
    def on_close(wss, close_status_code, close_msg):  # pylint: disable=unused-argument
        ...
        if close_status_code or close_msg:
            logger.debug(f"Close status code: {close_status_code}, Close message: {close_msg}")
