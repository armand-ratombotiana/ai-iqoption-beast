"""
Production-Ready Flask API for IQOption AI Binary Trading Bot - FIXED VERSION
...
CONFIG = {
    ...
    'MIN_TRADE_AMOUNT': float(os.getenv('MIN_TRADE_AMOUNT', 1))  # NEW: Minimum trade amount
}
...
def calculate_trade_amount(confidence, balance):
    ...
    confidence_factor = max(0.6, confidence / 100.0)  # Minimum 60% of base amount
...
def reset_state():
    ...
    global trading_state
    reset_type = request.json.get('type', 'daily') if request.json else 'daily'
...
if __name__ == '__main__':
    print(f"\n{'='*60}")
    print("IQOption AI Binary Trading Bot API - FIXED VERSION")
    print(f"{'='*60}")
