...
class IQ_Option:
    ...
    def get_trade_payout(self, trade_id):
        # Récupérer le payout pour un trade spécifique
        if trade_id in self.get_digital_spot_profit_after_sale_data:
            return self.get_digital_spot_profit_after_sale_data[trade_id]['payout']
        else:
            return None

    def get_trade_gain(self, trade_id):
        return self.get_digital_spot_profit_after_sale_data
    ...
    def get_all_open_time__00(self):
        ...
        # for digital
        print(self.get_digital_underlying_list_data())
    ...
    def get_all_open_time(self):
        ...
        for option in binary_list:
            for actives_id in binary_data[option]["actives"]:
                ...
                name = str(active["name"]).split(".")[1]
                if active["enabled"]:
                    OPEN_TIME[option][name]["open"] = not active["is_suspended"]
                else:
                    OPEN_TIME[option][name]["open"] = False

        # DIGITAL - Désactivé car la méthode est obsolète
        """
        try:
            digital_data = self.get_digital_underlying_list_data().get("underlying", [])
            for digital in digital_data:
                name = digital["underlying"]
                schedule = digital["schedule"]
                OPEN_TIME["digital"][name]["open"] = False
            ...
            print("⚠️ Erreur récupération des données digitales :", e)
        """
        ...
        for instruments_type in instrument_list:
            try:
                ...
                print(f"⚠️ Erreur instruments {instruments_type} :", e)
    ...
    def check_win_digital_11(self, buy_order_id, polling_time):
        while True:
            time.sleep(polling_time)
    ...
    def check_win_digital(self, buy_order_id, polling_time):
        while True:
            ...
            if "msg" not in data or "position" not in data["msg"]:
                continue

            position = data["msg"]["position"]
            if position["status"] == "closed":
                pnl_realized = position.get("pnl_realized")
                buy_amount = position.get("buy_amount")

                if pnl_realized is not None and buy_amount is not None:
                    if position["close_reason"] == "default":
                        return pnl_realized
                    elif position["close_reason"] == "expired":
                        return pnl_realized - buy_amount

                # Si pnl_realized ou buy_amount sont None, il y a un problème avec les données
                return None
